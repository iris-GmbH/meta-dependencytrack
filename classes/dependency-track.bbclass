...
def append_to_vex_vulnerabilities(vex, cve, state, bom_ref):
    ...
    for index, cpe in enumerate(oe.cve_check.get_cpe_ids(name, version)):
        ...
        if not next((c for c in sbom["components"] if c["cpe"] == cpe), None):
            ...
            for cve in cve_patched:
                ...
                # map Yoctos "Patched" to Dependencytracks "resolved"
                state = "resolved"
                append_to_vex_vulnerabilities(vex, cve, state, bom_ref)
            for cve in cve_ignored:
                bb.debug(2, f"Found ignore statement for CVE {cve} in {name}@{version}")
                # map Yoctos "Ignored" to Dependencytracks "not_affected"
                state = "not_affected"
                append_to_vex_vulnerabilities(vex, cve, state, bom_ref)
